...
func (s *PostgresStorage) Close() error {
	return fmt.Errorf("failed to close database connection: %w", s.store.Close())
}
...
func (s *PostgresStorage) validateScooterUpdate(current *Scooter, updated Scooter) error {
	...
	if updated.Status == string(StatusOccupied) && current.Status == string(StatusFree) && updated.ClientID == "" {
		return ErrClientIDRequired
	}

	// Prevent occupation of already occupied scooter by different client
	if updated.Status == string(StatusOccupied) && current.Status == string(StatusOccupied) && updated.ClientID != current.ClientID {
		return ErrScooterAlreadyOccupied
	}

	// Only allow release if scooter is occupied and client owns it.
	if updated.Status == string(StatusFree) && current.Status == string(StatusOccupied) && updated.ClientID != current.ClientID {
...
func (s *PostgresStorage) UpdateScooter(ctx context.Context, sc Scooter) error {
	...
	current, err := s.fetchScooterInTx(ctx, tx, sc.ID)
	if err != nil {
		return err
	}
	...
	if err := s.updateScooterInTx(ctx, tx, sc); err != nil {
		return err
	}
...
func (s *PostgresStorage) fetchScooterInTx(ctx context.Context, tx *sql.Tx, id uuid.UUID) (*Scooter, error) {
	query := `
		SELECT id, city, status, latitude, longitude, client_id, updated, created 
		FROM scooters
		WHERE id = $1
...
func (s *PostgresStorage) updateScooterInTx(ctx context.Context, tx *sql.Tx, sc Scooter) error {
	query := `
		UPDATE scooters 
	...
	if affected == 0 {
		return fmt.Errorf("%w: %s", ErrScooterStatusChanged, sc.ID)
	}
